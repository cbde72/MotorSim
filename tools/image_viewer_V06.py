...
from PySide6.QtCore import QByteArray, QFileSystemWatcher, QPointF, QRectF, QSettings, QSize, Qt, QTimer, Signal
...
class ImageLabel(QLabel):
    ...
    def set_pixmap(self, pixmap: QPixmap | None, *, reset_view: bool = True) -> None:
    ...
    def _image_rect(self) -> QRectF:
        ...
        center = QPointF(self.width() * 0.5, self.height() * 0.5) + self._pan_offset
        return QRectF(center.x() - width * 0.5, center.y() - height * 0.5, width, height)

    def mousePressEvent(self, event) -> None:  # type: ignore[override]
        if event.button() == Qt.MouseButton.LeftButton:
            self.clicked.emit()
        elif event.button() == Qt.MouseButton.MiddleButton and self.original_pixmap() is not None:
            self.clicked.emit()
    ...
    def wheelEvent(self, event) -> None:  # type: ignore[override]
        ...
        if abs(steps) < 1.0e-12:
            event.ignore()
            return
        old_zoom = self._zoom
        new_zoom = max(0.1, min(20.0, old_zoom * (1.2 ** steps)))
        if abs(new_zoom - old_zoom) <= 1.0e-12:
            event.accept()
            return

        # Keep the image point below the mouse cursor fixed while zooming.
        cursor = event.position()
        viewport_center = QPointF(self.width() * 0.5, self.height() * 0.5)
        old_center = viewport_center + self._pan_offset
        ratio = new_zoom / old_zoom
        new_center = cursor - (cursor - old_center) * ratio
        self._pan_offset = new_center - viewport_center
        self._zoom = new_zoom
        self.update()
        event.accept()
...
class ImagePane(QFrame):
    ...
    def copy_current_picture(self) -> bool:
    ...
    def signals_csv_path(self) -> Path | None:
        image_path = self.current_image_path()
        if image_path is None:
            return None
        # Result plots usually live in <result>/plots* while their source is
        # stored in <result>/csv/signals.csv.  Nearest match wins.
        for folder in (image_path.parent, *image_path.parents):
            candidates = (folder / "csv" / "signals.csv", folder / "signals.csv")
            for candidate in candidates:
    ...
    def _show_image_context_menu(self, position) -> None:
        self.activate()
        signals_path = self.signals_csv_path()
        menu = QMenu(self.image_label)
        action_open_signals = menu.addAction("signals.csv in Excel öffnen")
        action_open_signals.setEnabled(signals_path is not None)
        if signals_path is None:
            action_open_signals.setToolTip("Keine zugehörige csv/signals.csv gefunden")
        selected_action = menu.exec(self.image_label.mapToGlobal(position))
        if selected_action is action_open_signals and signals_path is not None:
            self._open_signals_in_excel(signals_path)

    def _open_signals_in_excel(self, signals_path: Path) -> None:
        try:
            os.startfile(str(signals_path))  # type: ignore[attr-defined]
        except OSError as exc:
            QMessageBox.critical(
                self,
                "signals.csv konnte nicht geöffnet werden",
                f"Die Datei konnte nicht mit Excel geöffnet werden:\n{signals_path}\n\n{exc}",
            )
    ...
    def _load_current_image(self, silent: bool = False) -> None:
        ...
        # The polling/file-watcher refresh must not destroy a user-selected
        # zoom and pan position.  Explicit image changes still start fitted.
        self.image_label.set_pixmap(pixmap, reset_view=not silent)
        self._update_watcher_paths()
        if not silent:
            self.activate()
...
class ImageViewerWindow(QMainWindow):
    ...
    def _build_actions(self) -> None:
        ...
        self.action_reload.setShortcut(Qt.Key.Key_F5)
        self.action_reload.triggered.connect(self.reload_active_pane)

        self.action_reset_image_view = QAction("Bildansicht zurÃ¼cksetzen", self)
        self.action_reset_image_view.setShortcut("Ctrl+R")
        self.action_reset_image_view.triggered.connect(self.reset_active_image_view)

        self.action_copy_picture = QAction("Aktuelles Bild kopieren", self)
        self.action_copy_picture.setShortcut(QKeySequence.StandardKey.Copy)
        self.action_copy_picture.triggered.connect(self.copy_active_picture)
    ...
    def _build_menu(self) -> None:
        ...
        menu_file.addAction(self.action_copy_picture)
        ...
        menu_view.addAction(self.action_reset_image_view)
    ...
    def _build_toolbar(self) -> None:
        ...
        toolbar.addAction(self.action_copy_picture)
        toolbar.addAction(self.action_reset_image_view)
        toolbar.addSeparator()
    ...
    def reset_active_image_view(self) -> None:
        pane = self._active_pane()
        pane.reset_image_view()
        self.status.showMessage(f"Bildansicht zurÃ¼ckgesetzt: {pane.current_status_text()}", 3000)

    def copy_active_picture(self) -> None:
        pane = self._active_pane()
        if pane.copy_current_picture():
            self.status.showMessage(f"Bild in die Zwischenablage kopiert: {pane.current_status_text()}", 3000)
        else:
            self.status.showMessage("Kein Bild zum Kopieren vorhanden.", 3000)
