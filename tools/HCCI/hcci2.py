...
class CanteraEngineSimulator:
    ...
    def run(self, t_end: float, num_points: int = 500) -> pd.DataFrame:
        ...
        for k, t_plot in enumerate(target_times):
            try:
                self.net.advance(float(t_plot))
            except Exception as e:
                print(f"[ABBRUCH] CVODE scheitert bei t={t_plot:.8e} s, Index={k}")
                print(e)
                break
            ...
            rows.append({
                ...
                "h_W_m2K": float(h),
                "Qdot_wall_W": float(h * self.geom.wall_area(theta_rad) * (T - self.cfg.wall_temperature_K)),
            })
