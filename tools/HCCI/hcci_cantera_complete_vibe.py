...
class EngineConfig:
    ...
    # Verbrennung
    # -------------------------------------------------------------------------
    # "cantera": detaillierte Chemie aus Mechanismus
    # "vibe":    Reaktionen deaktiviert, Energie wird ueber Vibe/Wiebe eingebracht
    # "off":     Reaktionen deaktiviert, keine Waermefreisetzung
    combustion_model: Literal["cantera", "vibe", "off"] = "vibe"

    # Vibe-/Wiebe-Brennverlauf, nur relevant bei combustion_model="vibe".
    vibe_start_deg: float = -5.0
    vibe_duration_deg: float = 35.0
    vibe_a: float = 6.908       # ca. 99.9 % Umsatz bei Brennende
    vibe_m: float = 2.0         # Formfaktor
    vibe_efficiency: float = 0.92
    fuel_lhv_J_kg: float = 42.7e6
...
def vibe_burn_fraction(theta_deg: float, cfg: EngineConfig) -> float:
    """Normierter Vibe-/Wiebe-Umsatz x_b(theta)."""
    if cfg.vibe_duration_deg <= 0.0:
        return 0.0
    phi = (theta_deg - cfg.vibe_start_deg) / cfg.vibe_duration_deg
    if phi <= 0.0:
        return 0.0
    if phi >= 1.0:
        return 1.0 - math.exp(-cfg.vibe_a)
    return 1.0 - math.exp(-cfg.vibe_a * phi ** (cfg.vibe_m + 1.0))


def vibe_dxbdtheta_rad(theta_deg: float, cfg: EngineConfig) -> float:
    """Ableitung dxb/dtheta in 1/rad."""
    if cfg.vibe_duration_deg <= 0.0:
        return 0.0
    phi = (theta_deg - cfg.vibe_start_deg) / cfg.vibe_duration_deg
    if phi <= 0.0 or phi >= 1.0:
        return 0.0
    duration_rad = math.radians(cfg.vibe_duration_deg)
    return (
        cfg.vibe_a
        * (cfg.vibe_m + 1.0)
        / duration_rad
        * phi ** cfg.vibe_m
        * math.exp(-cfg.vibe_a * phi ** (cfg.vibe_m + 1.0))
    )


def vibe_heat_release_W(theta_deg: float, omega_rad_s: float, m_fuel_kg: float, cfg: EngineConfig) -> float:
    """Vorgegebene Brennleistung nach Vibe/Wiebe."""
    if cfg.combustion_model != "vibe":
        return 0.0
    q_total_J = max(m_fuel_kg, 0.0) * cfg.fuel_lhv_J_kg * cfg.vibe_efficiency
    return q_total_J * vibe_dxbdtheta_rad(theta_deg, cfg) * omega_rad_s
...
class CanteraEngineSimulator:
    def __init__(self, gas: ct.Solution, geom: CrankSlider, cfg: EngineConfig):
        ...
        if cfg.combustion_model not in ("cantera", "vibe", "off"):
            raise ValueError("combustion_model muss 'cantera', 'vibe' oder 'off' sein.")

        # Bei Vibe/off wird die detaillierte Cantera-Chemie deaktiviert.
        # Die thermodynamischen Stoffwerte bleiben weiterhin aus Cantera.
        if cfg.combustion_model in ("vibe", "off"):
            for i in range(self.gas.n_reactions):
                self.gas.set_multiplier(0.0, i)

        self.initial_fuel_mass_kg = float(self.gas.Y[species_index_ci(self.gas, cfg.fuel_species)] * self.gas.density * geom.volume(math.radians(cfg.theta_start_deg)))
        ...
        self.wall.heat_flux = self.total_wall_heat_flux
    ...
    def wall_loss_heat_flux(self, t: float) -> float:
        theta = self.theta_from_t(t)
        T = float(np.clip(self.r.thermo.T, 250.0, self.cfg.max_temperature_K))
        ...
        # Positiver Flux transportiert Waerme aus dem Reaktor zur Umgebung.
        flux = h * (T - self.cfg.wall_temperature_K) * A_wall / A_ref
        if not np.isfinite(flux):
            return 0.0
        return float(flux)

    def total_wall_heat_flux(self, t: float) -> float:
        theta = self.theta_from_t(t)
        theta_deg = math.degrees(theta)
        A_ref = max(self.geom.wall_area(math.radians(self.cfg.theta_start_deg)), 1.0e-12)
        qdot_vibe = vibe_heat_release_W(theta_deg, self.omega, self.initial_fuel_mass_kg, self.cfg)

        # Vorzeichenkonvention Cantera-Wall:
        # +flux: Waerme vom Reaktor weg, -flux: Waerme in den Reaktor hinein.
        flux = self.wall_loss_heat_flux(t) - qdot_vibe / A_ref
        if not np.isfinite(flux):
            return 0.0
        return float(flux)
    ...
    def collect_row(self, t_plot: float) -> Dict[str, float]:
        ...
        hrr_chem_W_m3 = reaction_heat_release_W_per_m3(gas)
        hrr_chem_W = hrr_chem_W_m3 * V
        hrr_vibe_W = vibe_heat_release_W(math.degrees(theta_rad), self.omega, self.initial_fuel_mass_kg, self.cfg)
        hrr_W = hrr_chem_W + hrr_vibe_W
        ...
        return {
            ...
            "hrr_chem_W_m3": hrr_chem_W_m3,
            "hrr_chem_W": hrr_chem_W,
            "hrr_vibe_W": hrr_vibe_W,
            "hrr_W_m3": hrr_chem_W_m3,
            "hrr_W": hrr_W,
            "vibe_mfb": vibe_burn_fraction(math.degrees(theta_rad), self.cfg),
        }
...
def write_summary(df: pd.DataFrame, outdir: Path, cfg: EngineConfig, stop_reason: Optional[str]) -> None:
    ...
    lines.append(f"Verbrennungsmodell:   {cfg.combustion_model}")
    if cfg.combustion_model == "vibe":
        lines.append(f"Vibe Start/Dauer:     {cfg.vibe_start_deg:.3f} / {cfg.vibe_duration_deg:.3f} deg")
        lines.append(f"Vibe a/m:             {cfg.vibe_a:.3f} / {cfg.vibe_m:.3f}")
...
def main(cfg: EngineConfig = CFG) -> int:
    ...
    print(f"Verbrennungsmodell:   {cfg.combustion_model}")
    if cfg.combustion_model == "vibe":
        print(f"Vibe Start/Dauer:     {cfg.vibe_start_deg:.3f} / {cfg.vibe_duration_deg:.3f} deg")
        print(f"Vibe a/m:             {cfg.vibe_a:.3f} / {cfg.vibe_m:.3f}")
        print(f"Vibe Q_total:         {m_fuel * cfg.fuel_lhv_J_kg * cfg.vibe_efficiency:.3f} J")
