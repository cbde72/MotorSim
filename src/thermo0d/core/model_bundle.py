...
class FreePistonModelData:
    ...
    hcci_accumulation_start_mode_by_vol: np.ndarray = field(default_factory=lambda: np.zeros(0, dtype=np.int64))
    hcci_accumulation_start_distance_from_tdc_by_vol_m: np.ndarray = field(default_factory=lambda: np.zeros(0, dtype=np.float64))
    hcci_accumulation_end_mode_by_vol: np.ndarray = field(default_factory=lambda: np.zeros(0, dtype=np.int64))
