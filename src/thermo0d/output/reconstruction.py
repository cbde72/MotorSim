...
try:
    ...
    evaluate_free_piston_hcci_ignition_sample = None
    ...
    def build(cls, bundle, t: np.ndarray, y: np.ndarray, cycle_indices: np.ndarray, requested_keys=None) -> ReconstructedSeries:
        ...
        for k, tk in enumerate(t_arr):
            ...
            for i in range(n_vol):
                ...
                hcci_reason_code = 1.0
                hcci_accumulation_window_active = 0.0
                hcci_temperature_min_K = 0.0
                hcci_pressure_min_Pa = 0.0
                hcci_lambda = 0.0
                hcci_residual_fraction = 0.0
                hcci_tabulated_out_of_bounds = 0.0
                if comb_idx >= 0:
                    comb_row = comb_matrix[comb_idx]
                    duration_mode = int(combustion_duration_mode_from_row(comb_row))
                    if duration_mode == int(CombDurationMode.TIME):
                    ...
                    if fp is not None:
                        ...
                        q_total_for_delay = float(comb_row[int(CombCol.FUEL_MASS_PER_CYCLE)] * comb_row[int(CombCol.LHV)])
                        ...
                        if i in cylinder_latch_histories and float(cylinder_latch_histories[i][2][k]) > 0.0:
                            q_total_for_delay = float(cylinder_latch_histories[i][2][k])
                        if evaluate_free_piston_hcci_ignition_sample is not None:
                            sample = evaluate_free_piston_hcci_ignition_sample(
                                bundle,
                                i,
                                float(t_arr[k]),
                                state_k,
                                q_total_J=q_total_for_delay,
                                fuel_mass_kg=fuel_for_delay,
                                soc_active=bool(combustion_soc_energy_J > 0.0 and combustion_fraction < 1.0),
                                combustion_fraction_0to1=float(combustion_fraction),
                            )
                            hcci_reason_code = float(sample['reason_code'])
                            hcci_accumulation_window_active = 1.0 if bool(sample['window_active']) else 0.0
                            hcci_ignition_delay_s = float(sample['hot_tau_s'])
                            hcci_cool_ignition_delay_s = float(sample['cool_tau_s'])
                            hcci_lambda = float(sample['lambda'])
                            hcci_residual_fraction = float(sample['residual_fraction'])
                            hcci_tabulated_out_of_bounds = 1.0 if bool(sample['tabulated_out_of_bounds']) else 0.0
                        hcci_temperature_min_K = float(fp.hcci_start_temperature_min_by_vol_K[i]) if i < int(getattr(fp.hcci_start_temperature_min_by_vol_K, 'shape', (0,))[0]) else 0.0
                        hcci_pressure_min_Pa = float(fp.hcci_start_pressure_min_by_vol_Pa[i]) if i < int(getattr(fp.hcci_start_pressure_min_by_vol_Pa, 'shape', (0,))[0]) else 0.0
                        hcci_enabled = hcci_ignition_delay_s > 0.0
                        two_stage = hcci_cool_ignition_delay_s > 0.0
                        if hcci_enabled and hcci_accumulation_window_active > 0.0:
                ...
                cls._ensure_float_column(columns, f'{name}_hcci_validity_reason_code', n_samples)[k] = float(hcci_reason_code)
                cls._ensure_float_column(columns, f'{name}_hcci_accumulation_window_active_0to1', n_samples)[k] = float(hcci_accumulation_window_active)
                cls._ensure_float_column(columns, f'{name}_hcci_start_temperature_min_K', n_samples)[k] = float(hcci_temperature_min_K)
                cls._ensure_float_column(columns, f'{name}_hcci_start_pressure_min_Pa', n_samples)[k] = float(hcci_pressure_min_Pa)
                cls._ensure_float_column(columns, f'{name}_hcci_lambda', n_samples)[k] = float(hcci_lambda)
                cls._ensure_float_column(columns, f'{name}_hcci_residual_fraction_0to1', n_samples)[k] = float(hcci_residual_fraction)
                cls._ensure_float_column(columns, f'{name}_hcci_tabulated_out_of_bounds_0to1', n_samples)[k] = float(hcci_tabulated_out_of_bounds)
